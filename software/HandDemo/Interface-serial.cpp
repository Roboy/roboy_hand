...
void initSerialCharCodes(void)
{
  ...
  USBSERIAL_PRINTLN("Serial commands initalisiert");
...
bool checkSerial(void)
{
  ...
  if (Serial.available())
  {
    char rxChar = Serial.read();    // store char in temporary variable
...
void serial_ToggleDemoMode(int val)
{
  ...
  MYSERIAL_PRINTLN_PGM("Finger Roll");
  fingerRoll();
  fingerRoll();
  delay(1000);
  ...
  MYSERIAL_PRINTLN_PGM("Thumbs Up");
  thumbsUp();
  thumbsUp();
  delay(1000);

  MYSERIAL_PRINTLN_PGM("Open");
  openHand();
  delay(1000);

  MYSERIAL_PRINTLN_PGM("Pinch");
  pinch();
  pinch();
  delay(1000);
}
...
void serial_GripControl(int gNum)
{
  ...
  if(gNum == 0){
    USBSERIAL_PRINT_PGM("Grip:open_\n");
    openHand();
    
  }else if(gNum == 1){
    USBSERIAL_PRINT_PGM("Grip:close_\n");
    closeHand();
  }else if(gNum == 2){
    USBSERIAL_PRINT_PGM("Grip:thumbUp_\n");
    thumbsUp();
  }
  else if(gNum == 3){
    USBSERIAL_PRINT_PGM("Grip:thumbUp_\n");
...
void serial_MuscleControlMode(int mMode)
{
 /* const char *disabled_enabled[2] = { "DISABLED","ENABLED" };
  ...
  case 0:     // muscle mode off
    MYSERIAL_PRINT_PGM("TODO: ADD EMG MODE");
    //EMG.off();
    settings.mode = MODE_NONE;
    storeSettings();
...
void serial_SerialInstructions(int val)
{
  ...
  // TITLE
  USBSERIAL_PRINT_PGM("\n");
  USBSERIAL_PRINTLN_PGM("         Open Bionics Serial Commands - Beetroot");
  for (uint8_t i = 0; i < 60; i++)
  {
    USBSERIAL_PRINT_PGM("_");
  }
  USBSERIAL_PRINT_PGM("\n\n");

  // GRIPS
  USBSERIAL_PRINTLN_PGM("Grip Control (G#, K#)");
  USBSERIAL_PRINTLN_PGM("Command     Description");

  for (int gNum = 0; gNum < NUM_GRIPS; gNum++)
  {
    USBSERIAL_PRINT_PGM("G");
    USBSERIAL_PRINTLN(gNum);
    USBSERIAL_PRINT_PGM("         ");
    if (gNum < 10)            // fix alignment issue when digit only has one number (e.g. G3__### or G10_###)
    {
      USBSERIAL_PRINT_PGM(" ");
    }
    USBSERIAL_PRINTLN(Grip.getGripName(gNum));
    USBSERIAL_PRINT_PGM("\n");
  }
  USBSERIAL_PRINT_PGM("\n");

  // FINGER CONTROL
  USBSERIAL_PRINTLN_PGM("Finger Control (F#, P##, S##)");
  USBSERIAL_PRINTLN_PGM("Command     Description");
  USBSERIAL_PRINTLN_PGM("F#          Move finger, # is the finger number (F0 - F3)");
  USBSERIAL_PRINTLN_PGM("F# P50      Finger # to position 50 (P0 - P100)");
  USBSERIAL_PRINTLN_PGM("F# S255     Finger # at speed 255 (S0 - S255)");
  USBSERIAL_PRINT_PGM("\n");

  // MUSCLE CONTROL
  USBSERIAL_PRINTLN_PGM("Muscle Control (M#, U#, T#, N)");
  USBSERIAL_PRINTLN_PGM("Command     Description");
  USBSERIAL_PRINTLN_PGM("M0          Muscle control OFF");
  USBSERIAL_PRINTLN_PGM("M1          Standard muscle control ON");
  USBSERIAL_PRINTLN_PGM("M2          Muscle position control ON");
  USBSERIAL_PRINTLN_PGM("M3          Toggle whether to display SIMPLE muscle readings");
  USBSERIAL_PRINTLN_PGM("M4          Toggle whether to display DETAILED muscle readings");
  USBSERIAL_PRINT_PGM("\n");

  // ADVANCED SETTINGS
  USBSERIAL_PRINTLN_PGM("Advanced Settings (H#, A#, ?)");
  USBSERIAL_PRINTLN_PGM("Command     Description");
  USBSERIAL_PRINTLN_PGM("H           View hand configuration (LEFT or RIGHT)");
  USBSERIAL_PRINTLN_PGM("H1          Set hand to be RIGHT");
  USBSERIAL_PRINTLN_PGM("H2          Set hand to be LEFT");
  USBSERIAL_PRINTLN_PGM("A0          Toggle 'Automatic Demo Mode' ON/OFF");
  USBSERIAL_PRINTLN_PGM("A1          Toggle 'Serial Commands Instructions' ON/OFF");
  USBSERIAL_PRINTLN_PGM("A2          Toggle 'Wait For Serial Connection' ON/OFF");
  USBSERIAL_PRINTLN_PGM("A3          Enable/Disable motors");
  USBSERIAL_PRINTLN_PGM("A4          Enable/Disable CSV mode (fast control)");
  USBSERIAL_PRINTLN_PGM("A5          Enable/Disable HANDle mode (Wii Nunchuck)");
  USBSERIAL_PRINTLN_PGM("A6          Get the position of all fingers as a CSV string");
  USBSERIAL_PRINTLN_PGM("#           Display system diagnostics");
  USBSERIAL_PRINTLN_PGM("?           Display serial commands list");
  USBSERIAL_PRINT_PGM("\n");

  // EXAMPLES
  USBSERIAL_PRINT_PGM("\n\n");
  USBSERIAL_PRINTLN_PGM("Examples");
  USBSERIAL_PRINTLN_PGM("H2          Set hand to be configured as a LEFT hand");
  USBSERIAL_PRINTLN_PGM("G0 O        Fist Grip Open");
  USBSERIAL_PRINTLN_PGM("G2 C        Thumbs Up Close");
  USBSERIAL_PRINTLN_PGM("F0 O        Thumb Open");
  USBSERIAL_PRINTLN_PGM("F0          Toggle thumb direction (open/close)");
  USBSERIAL_PRINTLN_PGM("F1 P50      Index to position 50");
  USBSERIAL_PRINTLN_PGM("F2 P50 S80  Middle finger to position 50 at speed 80");
  USBSERIAL_PRINTLN_PGM("F0 O S200   Thumb Open at speed 200");
  USBSERIAL_PRINTLN_PGM("A0          Enable/Disable demo mode");
  USBSERIAL_PRINT_PGM("\n\n");

  // MODE ESCAPE TEXT
  //if (!settings.motorEn)      // enable motors
   // USBSERIAL_PRINTLN_PGM("Motors disabled, enter 'A3' to re-enable them");

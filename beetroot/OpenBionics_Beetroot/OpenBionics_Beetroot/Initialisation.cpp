...
void initFingerPins(void)
{
	...
	{     
		finger[fingerOrder[0]].attach(6, 9, A3, 0, fingerInv[0]);		// M1
    finger[fingerOrder[1]].attach(12, 5, A2, A5, fingerInv[1]);   // M2
    finger[fingerOrder[2]].attach(10, 11, A1, A9, fingerInv[2]);   // M3  
    finger[fingerOrder[3]].attach(3, 4, A0, 1, fingerInv[3]);   // M4
	...
	{
		finger[fingerOrder[3]].attach(6, 9, A3, 0, fingerInv[3]);		// M1     
		finger[fingerOrder[2]].attach(12, 5, A2, A5, fingerInv[2]);		// M2
		finger[fingerOrder[1]].attach(10, 11, A1, A9, fingerInv[1]);		// M3
		finger[fingerOrder[0]].attach(3, 4, A0, 1, fingerInv[0]);		// M4
	}
#else
